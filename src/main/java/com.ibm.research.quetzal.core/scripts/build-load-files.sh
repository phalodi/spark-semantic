...
function process_edge_sets() {
	...
	if [[ -d $WORKLOAD_DIR ]]; then
		...
		java -Xmx2048M $JAVA_OPTS -cp $CLASSPATH com.ibm.research.rdf.store.hashing.FindWorkloadProxies $WORKLOAD_DIR $nt_file_1.correlations > $nt_file_1.proxies
	    ...
	    if [[ -f $SYSTEM_PREDICATES ]]; then
		java -agentlib:jdwp=transport=dt_socket,server=y,address=7777,suspend=y  -Xmx2048M $JAVA_OPTS -cp $CLASSPATH com.ibm.research.rdf.store.hashing.AssignHashesToPredicates $nt_file.edge_sets $HASHES true $direct $SYSTEM_PREDICATES
